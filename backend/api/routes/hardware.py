...
# デバイスパスのバリデーションパターン
DEVICE_PATTERN = re.compile(r"^/dev/(sd[a-z]|nvme[0-9]n[0-9]|vd[a-z]|xvd[a-z]|hd[a-z])$")
...
async def get_smart(
    ...
    if not DEVICE_PATTERN.match(device):
        raise HTTPException(
            ...
            detail=f"Invalid device path: {device}. " "Allowed: /dev/sd[a-z], /dev/nvme[0-9]n[0-9], /dev/vd[a-z]",
        )
