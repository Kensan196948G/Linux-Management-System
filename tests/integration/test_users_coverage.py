...
class TestListUsersBody:
    ...
    def test_list_users_success(self, test_client, auth_headers):
        ...
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            ...
            resp = test_client.get("/api/users", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "success"
        assert data["total_users"] == 2

    def test_list_users_sudo_error_returns_403(self, test_client, auth_headers):
    ...
    def test_list_users_sudo_wrapper_exception_returns_500(self, test_client, auth_headers):
    ...
    def test_list_users_with_filter_locked_true(self, test_client, auth_headers):
    ...
    def test_list_users_with_username_filter(self, test_client, auth_headers):
...
class TestListUsersAlias:
    def test_list_alias_success(self, test_client, auth_headers):
        """エイリアス /list も同じ結果を返す"""
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            mock_sw.list_users.return_value = SUCCESS_USER_LIST
            resp = test_client.get("/api/users/list", headers=auth_headers)
        assert resp.status_code == 200
        assert resp.json()["status"] == "success"

    def test_list_alias_sudo_error(self, test_client, auth_headers):
...
class TestListGroupsAlias:
    def test_groups_alias_success(self, test_client, auth_headers):
        """エイリアス /groups も 200 を返す"""
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            mock_sw.list_groups.return_value = SUCCESS_GROUP_LIST
            resp = test_client.get("/api/users/groups", headers=auth_headers)
        assert resp.status_code == 200
        assert resp.json()["status"] == "success"

    def test_groups_alias_sudo_error(self, test_client, auth_headers):
        """エイリアスでも sudo エラーは 403"""
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
...
class TestGetUserDetail:
    def test_get_detail_success(self, test_client, auth_headers):
        """成功時に 200 とユーザー詳細を返す"""
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            mock_sw.get_user_detail.return_value = SUCCESS_USER_DETAIL
            resp = test_client.get("/api/users/alice", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "success"
        assert data["user"]["username"] == "alice"

    def test_get_detail_not_found(self, test_client, auth_headers):
        """sudo が status=error を返すと 404"""
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            mock_sw.get_user_detail.return_value = ERROR_RESULT
            resp = test_client.get("/api/users/nobody", headers=auth_headers)
        assert resp.status_code == 404

    def test_get_detail_sudo_exception(self, test_client, auth_headers):
    ...
    def test_get_detail_invalid_username_special_chars(self, test_client, auth_headers):
        """特殊文字を含むユーザー名は 400"""
        resp = test_client.get("/api/users/bad;user", headers=auth_headers)
...
class TestCreateUser:
    ...
    def test_create_user_success(self, test_client, admin_headers):
        ...
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            ...
            resp = test_client.post("/api/users", json=self.VALID_PAYLOAD, headers=admin_headers)
    ...
    def test_create_viewer_forbidden(self, test_client, viewer_headers):
        ...
        resp = test_client.post("/api/users", json=self.VALID_PAYLOAD, headers=viewer_headers)
        assert resp.status_code == 403

    def test_create_operator_forbidden(self, test_client, auth_headers):
        """operator は write:users 権限がないので 403"""
        resp = test_client.post("/api/users", json=self.VALID_PAYLOAD, headers=auth_headers)
        assert resp.status_code == 403
    ...
    def test_create_user_validate_username_raises(self, test_client, admin_headers):
        ...
        with patch("backend.api.routes.users.validate_username", side_effect=VErr("bad")):
            resp = test_client.post("/api/users", json=self.VALID_PAYLOAD, headers=admin_headers)
        assert resp.status_code == 400
...
class TestListGroups:
    def test_list_groups_success(self, test_client, auth_headers):
        ...
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            mock_sw.list_groups.return_value = SUCCESS_GROUP_LIST
            resp = test_client.get("/api/users/groups/list", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert data["status"] == "success"
        assert data["total_groups"] == 1

    def test_list_groups_sudo_error(self, test_client, auth_headers):
    ...
    def test_list_groups_sudo_exception(self, test_client, auth_headers):
        """SudoWrapperError は 500"""
        from backend.core.sudo_wrapper import SudoWrapperError
    ...
    def test_list_groups_sort_by_gid(self, test_client, auth_headers):
...
class TestCreateGroup:
    ...
    def test_create_group_success(self, test_client, admin_headers):
        ...
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            ...
            resp = test_client.post("/api/users/groups", json=self.VALID_PAYLOAD, headers=admin_headers)
        assert resp.status_code == 201
    ...
    def test_create_group_forbidden_name(self, test_client, admin_headers):
        ...
        if FORBIDDEN_GROUPS:
            ...
            resp = test_client.post("/api/users/groups", json=payload, headers=admin_headers)
            assert resp.status_code == 400
    ...
    def test_create_group_viewer_forbidden(self, test_client, viewer_headers):
        """viewer は 403"""
        resp = test_client.post("/api/users/groups", json=self.VALID_PAYLOAD, headers=viewer_headers)
        assert resp.status_code == 403
    ...
    def test_create_group_validate_groupname_raises(self, test_client, admin_headers):
        ...
        with patch("backend.api.routes.users.validate_groupname", side_effect=VErr("bad")):
            resp = test_client.post("/api/users/groups", json=self.VALID_PAYLOAD, headers=admin_headers)
        assert resp.status_code == 400
...
class TestDeleteGroup:
    def test_delete_group_success(self, test_client, admin_headers):
        ...
        with patch("backend.api.routes.users.sudo_wrapper") as mock_sw:
            ...
            resp = test_client.delete("/api/users/groups/oldgroup", headers=admin_headers)
        assert resp.status_code == 200
...
class TestModifyGroupMembership:
    ...
    def test_modify_member_validate_username_raises(self, test_client, admin_headers):
        ...
        original_validate_groupname = __import__(
        ...
        with patch("backend.api.routes.users.validate_groupname", side_effect=lambda v: None), \
