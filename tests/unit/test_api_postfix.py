...
class TestGetPostfixStatus:
    ...
    def test_status_success(self, test_client, admin_headers):
        ...
        with patch("backend.api.routes.postfix.sudo_wrapper") as mock_sw:
            ...
            response = test_client.get("/api/postfix/status", headers=admin_headers)
...
class TestGetPostfixQueue:
    ...
    def test_queue_success(self, test_client, admin_headers):
        """正常系: メールキュー取得"""
        mock_data = {"queue_size": 5, "messages": []}
        with patch("backend.api.routes.postfix.sudo_wrapper") as mock_sw:
            mock_sw.get_postfix_queue.return_value = mock_data
            response = test_client.get("/api/postfix/queue", headers=admin_headers)
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True

    def test_queue_wrapper_error(self, test_client, admin_headers):
        """SudoWrapperError 発生時は503"""
        with patch("backend.api.routes.postfix.sudo_wrapper") as mock_sw:
...
class TestGetPostfixLogs:
    ...
    def test_logs_success_default(self, test_client, admin_headers):
        """正常系: デフォルト行数でログ取得"""
        mock_data = {"lines": ["log1", "log2"], "count": 2}
        with patch("backend.api.routes.postfix.sudo_wrapper") as mock_sw:
            mock_sw.get_postfix_logs.return_value = mock_data
            response = test_client.get("/api/postfix/logs", headers=admin_headers)
        assert response.status_code == 200
        data = response.json()
        assert data["success"] is True

    def test_logs_success_custom_lines(self, test_client, admin_headers):
        """正常系: 行数指定"""
        mock_data = {"lines": ["log1"], "count": 1}
        with patch("backend.api.routes.postfix.sudo_wrapper") as mock_sw:
            mock_sw.get_postfix_logs.return_value = mock_data
            response = test_client.get(
                "/api/postfix/logs?lines=100", headers=admin_headers
            )
        assert response.status_code == 200

    def test_logs_invalid_lines_zero(self, test_client, admin_headers):
        """不正な行数（0）"""
        response = test_client.get(
            "/api/postfix/logs?lines=0", headers=admin_headers
        )
        assert response.status_code == 422

    def test_logs_invalid_lines_over(self, test_client, admin_headers):
        """不正な行数（201）"""
        response = test_client.get(
            "/api/postfix/logs?lines=201", headers=admin_headers
        )
        assert response.status_code == 422
